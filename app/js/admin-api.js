...
export const ADMIN_TABLES = [
  {
    ...
    fields: [
      ...
      { name: '_images', label: 'Görseller (her satıra bir URL)', type: 'textarea', virtual: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: '_images', label: 'Çoklu görseller (her satıra bir URL)', type: 'textarea', virtual: true },
...
export function getTableMeta(tableId) {
  return ADMIN_TABLES.find(table => table.id === tableId) || ADMIN_TABLES[0];
}

const IMAGE_TABLES = new Set(['announcements', 'projects', 'etkinlikler']);

function splitVirtualFields(tableId, data) {
  if (!IMAGE_TABLES.has(tableId)) return { data, images: null };
  const clean = { ...data };
  const images = parseImageList(clean._images);
  delete clean._images;
  return { data: clean, images };
}
...
async function attachAdminImages(tableId, records) {
  if (!IMAGE_TABLES.has(tableId) || !Array.isArray(records) || records.length === 0) return records;
  const ids = records.map(record => String(record.id)).filter(Boolean);
  const images = await adminRpc('admin_list_record_images', {
    p_entity_type: tableId,
    p_entity_ids: ids,
  });
  ...
  return records.map(record => ({ ...record, images: byRecord.get(String(record.id)) || [] }));
}

async function adminRpc(name, params) {
  const token = await getAdminToken();
  if (!token) throw new Error('Admin oturumu bulunamadi.');
  const { data, error } = await supabase.rpc(name, params, { token });
  if (error) throw error;
  return data;
}

export async function listRecords(tableId) {
  const records = await adminRpc('admin_list_records', { p_table: tableId });
  return attachAdminImages(tableId, Array.isArray(records) ? records : []);
}

export async function createRecord(tableId, data) {
  const payload = splitVirtualFields(tableId, data);
  const record = await adminRpc('admin_create_record', { p_table: tableId, p_data: payload.data });
  if (payload.images) {
    await adminRpc('admin_set_record_images', {
      p_entity_type: tableId,
...
export async function updateRecord(tableId, id, data) {
  const payload = splitVirtualFields(tableId, data);
  const record = Object.keys(payload.data).length
    ? await adminRpc('admin_update_record', { p_table: tableId, p_id: String(id), p_data: payload.data })
    : { id };
  if (payload.images) {
    await adminRpc('admin_set_record_images', {
      p_entity_type: tableId,
...
export async function deleteRecord(tableId, id) {
  return adminRpc('admin_delete_record', { p_table: tableId, p_id: String(id) });
}
